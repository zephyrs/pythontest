...
class Slope():
    ...
    def doFit(self):
        ...
        print(self._name + ':')
        print("%f x + %f y + %f = z" %
              (self._fit[0], self._fit[1], self._fit[2]))
        # print "errors:"
        # print self._errors
        # print "residual:"
    ...
    def doPlot(self, xlim, ylim):
        ...
        X, Y = np.meshgrid(np.arange(xlim[0], xlim[1]),
                           np.arange(ylim[0] - 5, ylim[1] + 5))
        Z = np.zeros(X.shape)
        for r in range(X.shape[0]):
            for c in range(X.shape[1]):
                Z[r, c] = self._fit[0] * X[r, c] + self._fit[1] * Y[
                    r, c] + self._fit[2]
...
def parseFile(fname):
    ...
    with open(fname, 'r') as f:
        ...
        pattern2 = re.compile(
            r"\bslope(?P<k>[12])--->\s+R_ref:\s+(?P<v>.*?)\s")
        for l in lines:
...
def runTest(fname):
    xs = [
        0, 59.5221, 91.1932, 80.1939, 31.6711, -31.6711, -80.1939, -91.1932,
        -59.5221
    ]
    #ys = [85.14, 85.15, 85.2, 85.2, 85.22, 85.14, 85.15, 85.5, 85.16]
    zs = [
        -92.6, -70.9357, -16.0798, 46.3, 87.0155, 87.0155, 46.3, -16.0798,
        -70.9357
    ]
