...
class ShootoutGame:
    ...
    def main(self):
        ...
        self.toss_coin()
        shooters = (
            (PlayerShooter(italy_players), ComputerShooter(france_players))
            if self._is_player_right
            else (ComputerShooter(italy_players), PlayerShooter(france_players))
        )
        ...
        while True:
            ...
            matchpoint = ShootoutGame.is_match_point((attacking_side, defending_side))
            self.shooting_action(attacking_side, defending_side, matchpoint)
            print(
                "(Round %d)    %s:%s - %d:%d"
                % (
                    attacking_side.round,
                    attacking_side.name,
                    defending_side.name,
                    attacking_side.score,
            ...
            if result != GameResult.Draw:
                print("YOU WIN!!!" if result == GameResult.PlayerWin else "YOU LOSE!!!")
                break
            i += 1
    ...
    def toss_coin(self):
        ...
        player_choise = input(
            "Coin tossing... choose Head('h') or Tail('t')) --> "
        ).rstrip()
    ...
    def shooting_action(self, shooter, keeper, matchpoint):
        ...
        keep_choise = ""
        while True:
            print("%s is stepping up to take the penalty..." % shooter.taker_name)
            if matchpoint > 0:
                print(
                    random.choice(
                        [
        ...
        if shoot_choise != keep_choise:
            ...
            print("shoot skill vs odds -> %d vs %d" % (shooter.taker_skill, odds))
            if shooter.taker_skill < odds:
                ...
                msg = "MISSED! " + random.choice(
                    [
                        ...
                        "He sends his kick high over the bar!",
                        "WAY HIGH!",
                    ]
                )
            else:
            ...
            print(
                "shooter vs keeper -> %d(%d, %f) vs %d(%d, %f)"
                % (
                    shooter.taker_skill * odds_shooter,
                    shooter.taker_skill,
                    odds_shooter,
        ...
        print(
            "\t%s shoot to %s, %s save to %s... %s"
            % (
                shooter.taker_name,
                directions_map[shoot_choise],
                keeper.keeper_name,
                directions_map[keep_choise],
                msg,
            )
        )
    ...
    def is_match_point(s):
        ...
        if ShootoutGame.judge_score((shooter_scored, keeper)) != GameResult.Draw:
            return 1

        shooter_not_scored = copy.deepcopy(shooter)
        shooter_not_scored.shoot(False)

        if ShootoutGame.judge_score((shooter_not_scored, keeper)) != GameResult.Draw:
            return -1
